...
/*64k*/
uint8_t buffer[STORAGE_BLK_NBR*STORAGE_BLK_SIZ] = MSC_DATA;
...
//    array_printf(buf,len);
    ...
    static int32_t flash_eraser_flag = 0;
    ...
    if(addr > addr_bak)
    {
        ...
        if(addr_bak >= 22016)
        {
          ...
          flash_port_write(flash_addr,(const uint32_t*)buf,len);
          flash_addr += len;
        }
    }
