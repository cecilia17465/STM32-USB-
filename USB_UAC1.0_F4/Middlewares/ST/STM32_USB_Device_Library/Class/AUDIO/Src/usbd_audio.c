...
/* USB Aduio 描述(18+1) */
  ...
  /* USB Speaker Class-specific AC Interface Descriptor */
  (8+2),//AUDIO_INTERFACE_DESC_SIZE,            /* bLength */
  AUDIO_INTERFACE_DESCRIPTOR_TYPE,      /* bDescriptorType */
  AUDIO_CONTROL_HEADER,                 /* bDescriptorSubtype */
  0x00,          /* 1.00 */             /* bcdADC */
  0x01,
  0x46,                                 /* wTotalLength = 30+30+(8+2)*/
  0x00,
  0x02,                                 /* bInCollection */
  0x01,                                 /* baInterfaceNr Speaker */
  0x02,                                 /* baInterfaceNr Mic */
  /* 09 byte*/
#endif
...
/* 录音设备流 音频流描述(52) */
  ...
  AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
  USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
  AUDIO_IN_EP,                         /* bEndpointAddress 1 input endpoint*/
  USBD_EP_TYPE_ISOC,                    /* bmAttributes */
  AUDIO_PACKET_SZE(USBD_AUDIO_FREQ),    /* wMaxPacketSize in Bytes (Freq(Samples)*2(Stereo)*2(HalfWord)) */
  0x01,                                 /* bInterval */
  0x00,                                 /* bRefresh */
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  (void)USBD_LL_OpenEP(pdev,AUDIO_IN_EP,USBD_EP_TYPE_ISOC,AUDIO_OUT_PACKET);
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  // UNUSED(epnum);
    ...
    if(epnum == (AUDIO_IN_EP & 0x0f))
    {
        ...
        (void)USBD_LL_Transmit(pdev,AUDIO_IN_EP,&haudio->buffer[haudio->rd_ptr],AUDIO_OUT_PACKET);
    }
...
static uint8_t USBD_AUDIO_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  if (haudio->control.cmd == AUDIO_REQ_SET_CUR)
  {
    ...
    if ((haudio->control.unit == AUDIO_OUT_STREAMING_CTRL) || (haudio->control.unit == AUDIO_IN_STREAMING_CTRL))
    {
